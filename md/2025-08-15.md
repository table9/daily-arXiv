...
- **TL;DR**: 提出了一种结合AVAS和AutoCAS方法的新方法，通过轨道熵分析和原子轨道投影统一指导活性空间选择，适用于强电子相关系统。
...
- **TL;DR**: 提出了一种基于大语言模型（LLM）的框架，通过动态访问代码模板和文档，实现无需编程经验的超表面逆向设计。
...
- **TL;DR**: 本文评估了基于SPICE-v2数据集训练的等变消息传递架构在生物分子模拟中的应用，发现模型大小、训练数据组成和静电处理对结果有影响，但长程静电作用对系统无系统性影响。
...
- **TL;DR**: 提出了一种高效、低内存的高斯和-张量神经网络（SOG-TNN）算法，用于求解高维薛定谔方程，通过低秩张量表示和SOG分解克服维度灾难。
...
- **TL;DR**: 该论文探讨了利用机器学习虚拟传感技术监测IGBT模块退化状态的可行性，通过有限物理传感器估算焊料层退化状态和温度分布，取得了高精度结果。
...
- **TL;DR**: 该论文构建了AA堆叠和AB堆叠扭曲SnSe2的相互作用Wannier模型，揭示了丰富的相关量子相。AA堆叠系统表现为三轨道三角晶格模型，而AB堆叠系统则实现了相互作用的kagome晶格模型。通过理论方法探索相图，发现了多种奇异量子相，包括二聚化相、价键固体和量子顺磁性。
...
- **TL;DR**: 本文提出了一种基于物理信息的深度对比源反演框架（DeepCSI），用于快速准确地重建不同测量条件下的介质参数，解决了传统非线性反散射问题的计算复杂性和多样性挑战。
...
- **TL;DR**: 研究了XX自旋链Y结的边界相图和低温热与磁化输运，发现两种强耦合机制，类似四通道和两通道Kondo固定点。XY各向异性会导致前者的不稳定性。
...
- **TL;DR**: 论文研究了MnBi2Te4薄膜中缺陷对量子反常霍尔效应和拓扑狄拉克态的影响，通过光谱椭偏和红外透射测量，发现电子局域化增强。
...
- **TL;DR**: 研究了RAlSi（R=La, Ce, Pr, Nd）的晶体电场（CEF）激发和磁性行为，发现CeAlSi和PrAlSi具有强单离子各向异性，而NdAlSi表现出竞争磁序。
...
- **Categories**: cond-mat.str-el, cond-mat.mtrl-sci
- **TL;DR**: 研究了6H-钙钛矿二聚体晶格Ba$_3$Zn$_{1-x}$Ca$_x$Ru$_2$O$_9$的磁性行为，结合理论分析和实验验证，揭示了磁性基态转变及磁矩抑制的三种机制。
...
- **TL;DR**: 提出了一种避免指数浓度问题的量子核方法（QKM），基于里德堡阻塞中的弱遍历破坏多体动力学，适用于当前中性原子量子计算机。
...
- **Categories**: cond-mat.str-el, quant-ph
- **TL;DR**: 论文分析了U(1)对称PEPS中规范变换的作用，提出了一种规范固定优化策略，以解决梯度优化中因规范自由度导致的能量计算不准确问题。
